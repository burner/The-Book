...
\begin{comment}
\begin{table}[H]
	\begin{tabular}{ p{2.00\textwidth}  p{2.00\textwidth} }
		 Example:To illustrate the dangers of macros, consider this naive macro\begin{verbatim}
	#define MAX(a,b) a>b?a:b
\end{verbatim}and the code
\begin{verbatim}
	i = MAX(2,3)+5;
	j = MAX(3,2)+5;
\end{verbatim}Take a look at this and consider what the the value after execution might be. The statements are turned into \begin{verbatim}
	int i = 2>3?2:3+5;
	int j = 3>2?3:2+5;
\end{verbatim}Thus, after execution \texttt{i=8} and \texttt{j=3} instead of the expected result of \texttt{i=j=8}! This is why you were cautioned to use an extra set of parenthesis above, but even with these, the road is fraught with dangers. The alert reader might quickly realize that if \texttt{a} or \texttt{b} contains expressions, the definition must parenthesize every use of \texttt{a,b} in the macro definition, like this:\begin{verbatim}
	#define MAX(a,b) ((a)>(b)?(a):(b))
\end{verbatim}This works, provided \texttt{a,b} have no side effects. Indeed, \begin{verbatim}
	i = 2;
	j = 3;
	k = MAX(i++, j++);
\end{verbatim}would result in \texttt{k=4}, \texttt{i=3} and \texttt{j=5}. This would be highly surprising to anyone expecting \texttt{MAX()} to behave like a function.So what is the correct solution? The solution is not to use macro at all. A global, inline function, like this\begin{verbatim}
	inline int max(int a, int b) { 
	  return a>b?a:b 
	}
\end{verbatim}has none of the pitfalls above, but will not work with all types.\{\textbar{} WIDTH=``95\%'' &  NOTE: The explicit \texttt{inline} declaration is not really necessary unless the definition is in a header file, since your compiler can inline functions for you (with gcc this can be done with \texttt{-finline-functions} or \texttt{-O3}). The compiler is often better than the programmer at predicting which functions are worth inlining. Also, function calls are not really expensive (they used to be). The compiler is actually free to ignore the \texttt{inline} keyword. It is only a hint (except that \texttt{inline} is necessary in order to allow a function to be defined in a header file without generating an error message due to the function being defined in more than one translation unit). \\
	\end{tabular}
\end{table}
...
The \textbf{\#} and \textbf{\#\#} operators are used with the \texttt{\#define} macro. Using \# causes the first argument after the \textbf{\#} to be returned as a string in quotes. For example, the command 
...
% commented out template(Wikipediapar)
\begin{comment}
\textbar{}C preprocessor
\end{comment}

% commented out template(reflist)

\href{http://en.wikibooks.org.org/wiki/de:C-Programmierung:_Präprozessor}{de:C-Programmierung: Präprozessor}
\href{http://en.wikibooks.org.org/wiki/fr:Programmation_C/Préprocesseur}{fr:Programmation C/Préprocesseur}
\href{http://en.wikibooks.org.org/wiki/it:C/Compilatore_e_precompilatore/Direttive}{it:C/Compilatore e precompilatore/Direttive}
\href{http://en.wikibooks.org.org/wiki/pl:C/Preprocesor}{pl:C/Preprocesor}
